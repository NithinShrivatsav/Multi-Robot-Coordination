...
N = 6;                        % Number of agents for cyclic pursuit
max_iter = 700;  
...
circularTargets = [ cos( 0:2*pi/N:2*pi*(1- 1/N) )/4 ; sin( 0:2*pi/N:2*pi*(1- 1/N) )/4 ];
    ...
    switch state
            ...
            for i = 1:N-1
                dx(:,i) = [0;0];
                    if (norm(x(:,i+1)-x(:,i))-interAgentDistance)>0
                        alpha = theta - (2.7*pi/18);
                        R = [cos(alpha) sin(alpha);-sin(alpha) cos(alpha)];
                    else
                        alpha = theta + (pi/18);
                        R = [cos(alpha) sin(alpha);-sin(alpha) cos(alpha)];
                    end
                    dx(:,i) = dx(:,i) + R*(x(:,i+1)-x(:,i))-((norm(x(:,i)-center)-radius)*(x(:,i)-center));
                ...
                dx(:,N) = dx(:,N) + R*(x(:,1)-x(:,N))-((norm(x(:,N)-center)-radius)*(x(:,N)-center));
        ...
        case 2
            ...
            while max(e_Norm) > 0.05
                ...
                refreshdata([delta_handle_p, wedge_handle_p, wedge_line_handle_left,wedge_line_handle_right]);
            end           % Set new velocities to robots and update
            state = 3;           
        ...
        case 3
            ...
            dx(:,1) = dx(:,1) + R*(x(:,N)-x(:,1))-((norm(x(:,1)-center)-radius)*(x(:,1)-center));          
