...
%           dx(:,defenceProtectors(3)) = dx(:,defenceProtectors(3))  + 0.5 * norm(x_int(:,defenceProtectors(1))-x_int(:,defenceProtectors(3))-0.2) * ( x_int(:,defenceProtectors(1)) - x_int(:,defenceProtectors(3)) ) + 0.5 * norm(x_int(:,defenceProtectors(2))-x_int(:,defenceProtectors(3))-0.2) * ( x_int(:,defenceProtectors(2)) - x_int(:,defenceProtectors(3)) );    
    ...
    switch(statesDecoyers)
        case 1
            ...
            if norm([0.6; 0] - x_int(:, NP+4)) > 0.02
                dx(:, NP+4) = [0.45; 0.8] - x_int(:, NP+4);
            end                
            subteam_goals = [0.0, 0.1; -0.6, 0.5];
            leaders = [NP+2];
            for i = 1:length(leaders)                
                if norm(subteam_goals(:,i) - x_int(:,leaders(i))) >=0.01
                    dx(:,leaders(i)) = dx(:,leaders(i)) + 0.5*(subteam_goals(:, i) - x_int(:,leaders(i)))/norm(subteam_goals(:, i) - x_int(:,leaders(i)));
                else
            ...
            for i = NP+1 : NP+3
                for j = NP+1 : NP+3
                    if i~=j
                        ...
                        weight = (1 - (deltaSeparation / norm ( x_int(:,i) -  x_int(:,j) )) )/(disk_delta - norm ( x_int(:,i) -  x_int(:,j) ))^3;
                        dx(:,i) = dx(:,i) + 0.1 * weight * ((x_int(:,j) - x_int(:,i)));
                    end
                end
            end
        ...
        case 2
            ...
            for i = [NP+4]'
                if(isempty(find(L(i, 1:NP))))
                    dx(:, i) = flagpos - x_int(:, i);
                else
                    decoypoint = [-1; 0];
                    if(length(find(L(i, 1:NP))) == 1)
                        disp('case 4 else')
                        j = find(L(i, 1:NP));
                        dx(:, i) = (norm(x_int(:, j) - x_int(:, i)) - 0.15)*(x_int(:, j) - x_int(:, i)) -0.2*(decoypoint - x_int(:, i))/norm(decoypoint - x_int(:, i));
                    end
                end
            end
    ...
    dx(:,NP+1:N) = si_to_uni_dyn(dx(:,NP+1:N), x(:,NP+1:N));
        
    dxmax = 0.1;
    dx(:,NP+1) = dx(:,NP+1)/norm(dx(:,NP+1))*dxmax;
    dx(:,NP+2) = dx(:,NP+2)/norm(dx(:,NP+2))*dxmax;
    dx(:,NP+3) = dx(:,NP+3)/norm(dx(:,NP+3))*dxmax;
    dx(:,NP+4) = dx(:,NP+4)/norm(dx(:,NP+4))*dxmax;
