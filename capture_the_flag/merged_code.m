...
rectangle('Position',[-0.6 -1 2.2 2], 'LineWidth', 3, 'EdgeColor', 'b');
...
targetsDecoyers = [-ones(1, ND); linspace(-0.5, 0.5, ND)];
...
for t = 1:iterations    
    ...
    switch (statesDecoyers)
        ...
        for i = NP+1 :N
            ...
            if (i==NP+2 && length(topological_neighbors(L,i))>=2)
                checkLeader1 = 1;
            end
            if (i==NP+5 && length(topological_neighbors(L,i))>=2)
                checkLeader2 = 1;
            end
        end
        ...
        for i = NP+1:N
            for j = topological_neighbors(L,i)
                error(i) = error(i) + ( norm(x(:,j) - x(:,i)) - Wd(i - NP ,j - NP)); 
                dx(:,i) = dx(:,i) + (x(:,j) - x(:,i)) * (norm(x(:,j) - x(:,i)) - Wd(i - NP,j - NP));
            end
            
        end
        
        if (sum(abs(error))<0.05)
            statesDecoyers = 2;
        end
        ...
        case 2  
            ...
            for i = NP+1 : NP+3
                for j = NP+1 : NP+3
                    if i~=j
                        dx(:,i) = dx(:,i) + (2*disk_delta - (norm (x(:,i) - x(:,j)) )) / (disk_delta - (norm (x(:,i) - x(:,j)) ))^2 *( (x(:,j) - x(:,i)));
                    end
                end
            end
